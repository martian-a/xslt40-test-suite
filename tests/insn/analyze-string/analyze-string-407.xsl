   ...
   <t:template match="date">
      ...
      <t:analyze-string select="." regex="{$regex}">
         <t:matching-substring>
            <t:number value="regex-groups()?year?value" format="0001"/>
            <t:text>-</t:text>
            <t:number value="index-of($months, regex-groups()?month?value)" format="01"/>
            <t:text>-</t:text>
            <t:number value="regex-groups()?day?value" format="01"/>
         </t:matching-substring>
      </t:analyze-string>
   </t:template>
</t:transform>
